...
interface RegisterUser extends Omit<User, "_id"> {
  password: string
...
export function Register({
  ...
  const handleRegister = (e: FormEvent) => {
    ...
    if (user.password.length < 4) {
      setError("Password must have 4 digit or character!")
    }
    mutate(user);
  }
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="overflow-hidden p-0">
        <CardContent className="grid p-0 md:grid-cols-2">
          ...
          <div className="relative hidden bg-muted sm:flex justify-center items-center flex-col">
            <BookIcon className="size-10" />
            <h2 className="text-2xl">CourseWala</h2>
          </div>        </CardContent>
      </Card>
